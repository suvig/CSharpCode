...
class Program
{
    static void Main()
    {
        ...
        string manifestData = System.IO.File.ReadAllText("SampleData/SampleManifest.xml");//<---COMMENT ME OUT WHEN TESTING IN CLM!
        ...
        foreach (XmlNode currentRowNode in manifest.SelectNodes("//" + colRowIterator)){
                ...
                // If not an Xml source, handle as string variable, but we'll put it in XML format for consistent processing downstream
                        ...
                        try
                        {
                                ...
                                string testStringData = System.IO.File.ReadAllText("SampleData/SimpleVariableSimulated.xml");
